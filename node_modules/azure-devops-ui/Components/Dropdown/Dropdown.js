...
import { getListBoxItemsValue, getUnselectableRanges, wrapListBoxItems } from '../../ListBox';
...
var Dropdown = /** @class */ (function (_super) {
    ...
    function Dropdown(props) {
        ...
        _this.onActivate = function (event, item) {
            if (!event.defaultPrevented && event.type === "keydown") {
                if (_this.state.filteredSelection.multiSelect) {
                    _this.state.filteredSelection.toggle(_this.state.filteredItems.value.indexOf(item), _this.state.filteredSelection.alwaysMerge);
                }
                else {
                    _this.state.filteredSelection.select(_this.state.filteredItems.value.indexOf(item), 1, _this.state.filteredSelection.alwaysMerge);
                }
        ...
        _this.onSelect = function (event, item) {
            ...
            if (dismissOnSelect !== undefined ? dismissOnSelect : selection.value.length > 0 && !selection.multiSelect && !selection.selectOnFocus) {
                _this.onDismiss();
            }
        };
        ...
        _this.renderCallout = function (dropdown, dropdownId, anchorElement, anchorOffset, anchorOrigin, anchorPoint, dropdownOrigin) {
            var _a = _this.props, actions = _a.actions, calloutContentClassName = _a.calloutContentClassName, containerClassName = _a.containerClassName, filterPlaceholderText = _a.filterPlaceholderText, filteredNoResultsText = _a.filteredNoResultsText, getUnselectableRanges = _a.getUnselectableRanges, items = _a.items, loading = _a.loading, noItemsText = _a.noItemsText, renderItem = _a.renderItem, searching = _a.searching, showFilterBox = _a.showFilterBox, userFilteredItems = _a.userFilteredItems;
            var width = _this.props.width;
            if (width === undefined && _this.expandableContainer.current) {
                width = _this.expandableContainer.current.clientWidth;
            }
        ...
        _this.state = {
            filteredItems: new ObservableArray(itemsValue.slice()),
    ...
    Dropdown.getDerivedStateFromProps = function (props, state) {
        ...
        return tslib_1.__assign({}, state, { props: props, wrappedItems: wrapListBoxItems(props.items) });
    };
    ...
    Dropdown.prototype.render = function () {
        var _this = this;
        var _a = this.props, ariaLabel = _a.ariaLabel, className = _a.className, disabled = _a.disabled, items = _a.items, placeholder = _a.placeholder, onCollapse = _a.onCollapse, renderExpandable = _a.renderExpandable, renderSelectedItems = _a.renderSelectedItems;
...
export function filterItemByText(filterText, item) {
    if (item.text && item.type !== 2 /* Header */ && item.type !== 3 /* Divider */ && item.type !== 4 /* Loading */) {
...
function updateFilteredItems(props, state) {
    ...
    filteredSelection.updateFilteredSelection(filteredIndexMap);
    return true;
}
...
export function renderHighlightedText(rowIndex, columnIndex, tableColumn, tableItem, filterResults) {
    ...
    if (filterResults && tableItem.text) {
        item = tslib_1.__assign({}, tableItem, { textNode: getHighlightedText(tableItem.text, filterResults) });
    }
    return (React.createElement(SimpleTableCell, { className: css(tableColumn.className, tableItem.className, tableItem.type === 2 /* Header */ && "bolt-list-box-header"), columnIndex: columnIndex, key: columnIndex, tableColumn: tableColumn },
        React.createElement("div", { id: tableItem.type === 2 /* Header */ ? "header-" + tableItem.id : undefined, "aria-label": tableItem.type === 2 /* Header */ ? format(Resources.HeaderAriaLabel, tableItem.text) : undefined }, renderListCell(item))));
}
export function getHighlightedText(text, matchingIndices) {
    var splitText = [];
    ...
    return React.createElement("span", null, formattedText);
}
