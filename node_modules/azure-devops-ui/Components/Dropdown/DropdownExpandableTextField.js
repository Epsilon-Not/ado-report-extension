...
export function DropdownExpandableTextField(props) {
    var editable = props.editable, expandableRef = props.expandableRef, items = props.items, inputClassName = props.inputClassName, renderSelectedItems = props.renderSelectedItems, selection = props.selection, _a = props.showPrefix, showPrefix = _a === void 0 ? true : _a, rest = tslib_1.__rest(props, ["editable", "expandableRef", "items", "inputClassName", "renderSelectedItems", "selection", "showPrefix"]);
    var prefixIconProps;
    var text;
    if (!editable) {
    ...
    if (showPrefix && selection.selectedCount === 1) {
        prefixIconProps = items[selection.value[0].beginIndex].iconProps;
    }
    return (React.createElement(ExpandableTextField, tslib_1.__assign({ ariaHasPopup: "menu", ariaRoleDescription: !editable ? Resources.DropdownExpandableRoleDescription : undefined, autoComplete: editable, ariaAutoComplete: editable ? "list" : undefined, focusTreatment: editable ? 0 /* all */ : 1 /* keyboardOnly */, inputType: !editable ? "button" : undefined, editable: editable, ref: expandableRef, value: text }, rest, { className: css(props.className, "bolt-dropdown-expandable-text-field", rest.disabled && "disabled"), inputClassName: css(inputClassName, "bolt-dropdown-expandable-textfield-input text-ellipsis"), onClick: function (event) { return expandableRef.current && expandableRef.current.expand(); }, prefixIconProps: props.prefixIconProps || prefixIconProps, tooltipProps: tslib_1.__assign({ text: text, overflowOnly: true, overflowDetected: overflowDetected }, props.tooltipProps) })));
}
