...
import { renderHighlightedText } from "../Dropdown/Dropdown";
var CustomEditableDropdown = /** @class */ (function (_super) {
    tslib_1.__extends(CustomEditableDropdown, _super);
    function CustomEditableDropdown(props) {
        ...
        _this.renderItem = function (rowIndex, columnIndex, tableColumn, tableItem) {
            return (React.createElement(Observer, { focusedIndex: {
                ...
                }, key: tableItem.id }, function () {
                var item = _this.filteredIndexMap.value[rowIndex] === _this.focusedIndex.value
                    ? tslib_1.__assign({}, tableItem, { className: css(tableItem.className, "bolt-editable-dropdown-focused-item") }) : tslib_1.__assign({}, tableItem);
                ...
                return _this.props.renderItem(rowIndex, columnIndex, tableColumn, item);
            }));
        };
        _this.onCollapse = function () {
            var _a = _this.props, allowFreeform = _a.allowFreeform, autoAccept = _a.autoAccept, onCollapse = _a.onCollapse, onTextChange = _a.onTextChange, onValueChange = _a.onValueChange, text = _a.text;
            if (onCollapse) {
                onCollapse();
            }
            ...
            if (allowFreeform &&
                ...
                if (autoAccept || _this.selectedItemInList) {
        ...
        _this.renderExpandable = function (props) {
            return (React.createElement(Observer, { focusedIndex: _this.focusedIndex, text: _this.props.text }, function (observerProps) {
                var onTextChange = _this.props.onTextChange;
                var focusedIndex = observerProps.focusedIndex, text = observerProps.text;
                var activeId;
                if (focusedIndex > -1 && _this.itemProvider.value[focusedIndex]) {
                    activeId = _this.itemProvider.value[focusedIndex].id;
                }
                var expandableProps = tslib_1.__assign({}, props, { ariaActiveDescendant: activeId, editable: true, showPrefix: text ? false : true, blurDismiss: true, onChange: onTextChange, onKeyDown: _this.onKeyDown, value: text });
                return _this.props.renderExpandable(expandableProps);
            }));
        };
        _this.renderCallout = function (props) {
            var calloutProps = tslib_1.__assign({}, props, { focusOnMount: false, excludeTabStop: true, excludeFocusZone: true, ignoreMouseDown: true, listBoxRef: _this.listBox });
        ...
        _this.onValueChange = function () {
            ...
            if (_this.props.allowFreeform) {
                if (_this.itemProvider.hasExtraItem) {
                    // Remove from the freeform item from the filtered set first so we don't
        ...
        _this.onKeyDown = function (ev) {
            ...
            switch (keyCode) {
                ...
                case 9 /* tab */:
                    if (_this.isExpanded && !ev.shiftKey && _this.filteredItems.length) {
                        _this.selectIndex(_this.focusedIndex.value);
                        _this.collapse();
                        ev.preventDefault();
                    }
                    break;
                ...
                case 46 /* delete */:
                case 8 /* backspace */:
                    if (!ObservableLike.getValue(_this.props.text || "")) {
                        _this.selection.clear();
        ...
        _this.filteredItems = new ObservableArray(_this.itemProvider.value.slice());
        ...
        if ($DEBUG && _this.selection.multiSelect) {
    ...
    CustomEditableDropdown.prototype.render = function () {
        var _this = this;
        var _a = this.props, ariaLabel = _a.ariaLabel, calloutContentClassName = _a.calloutContentClassName, className = _a.className, disabled = _a.disabled, filterByText = _a.filterByText, getUnselectableRanges = _a.getUnselectableRanges, noItemsText = _a.noItemsText, selectedText = _a.selectedText, text = _a.text;
        return (React.createElement(Observer, { text: { observableValue: text, filter: this.onValueChange }, items: { observableValue: this.itemProvider, filter: this.onItemsChange }, selection: this.selection, selectedText: selectedText }, function (props) {
            ...
            return (React.createElement(Dropdown, { ariaLabel: ariaLabel, calloutContentClassName: calloutContentClassName, className: css("bolt-editable-dropdown", (props.selection.length > 0 || !!props.selectedText) && "bolt-editable-dropdown-with-selection", className), disabled: disabled, getUnselectableRanges: getUnselectableRanges, filterByText: filterByText, items: _this.itemProvider, noItemsText: noItemsText || Resources.NoItemsFound, onCollapse: _this.onCollapse, onExpand: _this.onExpand, onSelect: _this.onSelect, placeholder: placeholder, ref: _this.dropdown, renderCallout: _this.renderCallout, renderExpandable: _this.renderExpandable, renderItem: _this.renderItem, selection: _this.selection, showFilterBox: false, userFilteredItems: _this.filteredItems, userFilteredItemsIndexMap: _this.filteredIndexMap }));
        }));
    };
    ...
    CustomEditableDropdown.prototype.updateFilteredIndexMap = function (filteredIndexMap) {
        ...
        if (!focusedIndex ||
            ...
            focusedIndex = this.filteredIndexMap.value.find(function (item) { return _this.selection.selectable(item); });
        }
        return focusedIndex;
    };
