...
var List = /** @class */ (function (_super) {
    ...
    function List(props) {
        ...
        _this.onIntersect = function (entries) {
            ...
            var _a = _this.state, firstRendered = _a.firstRendered, firstMaterialized = _a.firstMaterialized, lastRendered = _a.lastRendered, lastMaterialized = _a.lastMaterialized;
    ...
    List.prototype.focusRow = function (rowIndex, direction) {
        var _this = this;
        this.scrollIntoView(rowIndex, { block: "center" }, function (completedIndex) {
        ...
        if (!selection || selection.selectable(listRow.index)) {
            if (selection) {
                ...
                if (this.pivotIndex >= 0 && event.shiftKey && selection.multiSelect) {
                    ...
                    var isSpaceBarStroke = event.which === 32 /* space */;
                    if ((event.ctrlKey || event.metaKey || selection.alwaysMerge || isSpaceBarStroke) && selection.multiSelect) {
    ...
    List.prototype.renderSpacer = function (key, rowCount) {
        ...
        return (React.createElement("tr", { "aria-hidden": "true", className: "bolt-list-row-spacer invisible", key: key, ref: function (spacerElement) {
            ...
            React.createElement("td", { className: "bolt-list-cell-spacer invisible", colSpan: this.props.columnCount, style: { height: rowCount * this.state.rowHeight * this.state.rowProportion + "px" } })));
    };
...
export function ListItem(props) {
    ...
    return (React.createElement(FocusWithin, { onFocus: onFocus }, function (focusStatus) { return (React.createElement(FocusZoneContext.Consumer, null, function (rowContext) {
        return (React.createElement(FocusZone, { direction: 1 /* Horizontal */ },
            React.createElement(RowType, tslib_1.__assign({}, linkForwardProps, { "aria-busy": ariaBusy, "aria-describedby": ariaDescribedBy, "aria-label": ariaLabel, "aria-posinset": ariaPosInSet === undefined ? index + 1 : ariaPosInSet === null ? undefined : ariaPosInSet, "aria-selected": selection && selection.selected(index), "aria-setsize": ariaSetSize === undefined
                    ? props.details.listProps.itemProvider.length
                    : ariaSetSize === null
                        ? undefined
                        : ariaSetSize, className: css(props.className, "bolt-list-row", index === 0 && "first-row", linkProps && "bolt-link", selection && selection.selected(index) && "selected", focusStatus.hasFocus && "focused", singleClickActivation && "single-click-activation", selectableText && "selectable-text"), "data-focuszone": excludeFocusZone || (selection && !selection.selectable(index)) ? undefined : rowContext.focuszoneId, "data-row-index": index, tabIndex: getTabIndex(details), onBlur: focusStatus.onBlur, onFocus: focusStatus.onFocus, role: selection ? "option" : "listitem" }),
...
export function renderListCell(listCell) {
    ...
    var textContent = (React.createElement(Tooltip, { overflowOnly: true },
        React.createElement("span", { className: "text-ellipsis body-m" }, typeof listCell === "string" || typeof listCell === "number" ? listCell : listCell.textNode ? listCell.textNode : listCell.text)));
    ...
    if (typeof listCell !== "string" && typeof listCell !== "number") {
        ...
        if (listCell.iconProps) {
            content = (React.createElement(React.Fragment, null,
                Icon(tslib_1.__assign({}, listCell.iconProps, { className: css("icon-margin", listCell.iconProps.className) })),
                textContent));
        }
        if (listCell.href) {
